...
## Request message for the next AI agent

Copy the following message into the next AI-agent task:

---

Please resume the APDC attendance assistant architectural review in:

`D:\projects\llm_engineering_ed_donner\llm_engineering`

Start from the current `main` branch and read these documents completely before
making changes:
...
The final target of the plan is correct, relevant, and complete answers produced
by architectural, consistent, pattern-based code and code design. Do not treat
this as an off-day, `Day_Type`, or any other single-column issue. Do not add
question-specific prompt rules, hardcoded answer mappings, silent fallbacks, or
compatibility workarounds.

Perform these two review tracks step by step:

1. Review every architectural change implemented by the schema-grounded query
   plan. Find changes that can produce incomplete, irrelevant, or inaccurate AI
   answers, or that prevent the assistant from handling important categories of
   questions. Trace actual runtime paths and contracts manually, create focused
   regressions for every confirmed gap, identify the root cause, and fix all
   architectural issues correctly and natively. Large changes are authorized
   when genuinely necessary for a coherent design; do not preserve a flawed
   abstraction merely to keep the diff small.

2. Manually compare the current implementation with commit `44b8413a`, which is
   the requested pre-plan comparison baseline. Review the diff and execute both
   versions' relevant behavior paths step by step. Ensure every architectural
   change is better than the baseline and improves correctness, relevance,
   completeness, maintainability, extensibility, readability, consistency, and
   predictability. If any behavior or capability regressed, determine the root
   reason and implement the necessary native fix. Do not restore obsolete code
   blindly; preserve useful prior behavior by expressing it through the current
   standardized architecture.

Required working method:

- Begin with a clean-worktree and baseline verification.
- Inspect the code and `git diff 44b8413a..HEAD` manually; do not rely only on
  summaries, automated review, or existing tests.
- Build a question-capability matrix covering all fields/resolution kinds plus
  measures, predicates, calculations, grouping, ordering, ranking, limits,
  percentages, record projection, semantic narratives, employee resolution,
  clarification, and follow-ups.
- For every suspected issue, reproduce it and identify the root cause before
  editing. Add a failing regression, then implement the architectural fix.
- Keep the registry, semantic resolver, invariant compiler, executable plan,
  PostgreSQL compiler, Chroma behavior, answer contract, and evaluation model
  consistent with one another.
- Reject unsupported language explicitly and safely rather than executing a
  partially understood plan.
- Remove newly discovered stale or duplicate code only after proving it has no
  live caller or required behavior.
- Run focused tests continuously, then the complete unit/integration suites,
  Ruff, compilation, dataset verification, and the full provider-backed behavior
  corpus when provider access is healthy.
- Treat provider failures as inconclusive; never report them as product passes
  or product regressions without evidence.
- Protect private APDC data and use only the configured repository remote.
- Commit coherent changes, push `main`, and leave the worktree clean when the
  review and fixes are complete.

Report the manual comparison findings, confirmed root causes, architectural
fixes, remaining unsupported capabilities, verification results, commit hash,
and clean-worktree status.
